...
from Ui_tcEnable import Ui_Dialog
#-------------------------------------------------------------------------------------------------------------------
# Global Variables
...
class tcEnable(QDialog, Ui_Dialog):
    def __init__(self, parent=None):
        super(tcEnable, self).__init__(parent)
        self.setupUi(self)
  ...
  #----------------------------------------------------
    def readUart(self):
        global  inmsg
        if  os.path.exists(tcSe)==True:  
            with open(tcSe, 'rb') as f:
                f.readinto(inmsg)
            f.close()

            tcSSize=os.path.getsize(tcSe)
            ...
            if (value1[0]==0x7e):
                ...
                if((value1[5]==0x19)or(value1[5]==0x1b)):  

                       
                    if(value1[5]==0x19):

                        QMessageBox.question(self, 'Save TC Enable Status', "Save TC Enable Successful.", QMessageBox.Ok)
                        return
                    ...
                    #raw1:
                     ...
                     #raw3:
                        ...
                        QMessageBox.question(self, 'Get TC Enable Status', "Get TC Enable  Successful.", QMessageBox.Ok)
                        return
        else:   
            #update enable:
...
#----------------------------------------------------------- 
    ...
    def on_enableAllBu_clicked(self):
        ...
        for i in range(35):
           enable[i]=1
        f = open(tcE  ,'wb')     #TC enable <-> TC table
        f.write(enable)
        f.close()
